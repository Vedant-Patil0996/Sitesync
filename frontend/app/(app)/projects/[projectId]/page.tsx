...
export default function ProjectDetailPage() {
  ...
  const saveProject = async () => {
    await apiFetch(`/api/v1/projects/${projectId}`, {
      method: 'PATCH',
      body: JSON.stringify({ name: editName, budget_allocated: Number(editBudget), site_id: projectData.site_id, pm_id: projectData.pm_id, start_date: projectData.start_date || null, end_date: projectData.end_date || null, status: projectData.status }),
    });
    setEditing(false);
    const data = await apiFetch<any>(`/api/v1/projects/${projectId}`);
    setProjectData(data);
  ...
  const saveSchedule = async () => {
    await apiFetch(`/api/v1/projects/${projectId}/schedule`, {
      method: 'PATCH',
      body: JSON.stringify({ start_date: projectData.start_date || null, end_date: projectData.end_date || null, status: projectData.status }),
    });
    await reloadProject();
  };
  ...
  const createTask = async () => {
    ...
    const result = await apiFetch<any>(`/api/v1/projects/${projectId}/tasks`, {
      method: 'POST',
      body: JSON.stringify({ ...taskForm, assigned_to: taskForm.assigned_to ? Number(taskForm.assigned_to) : null, depends_on_task_id: taskForm.depends_on_task_id ? Number(taskForm.depends_on_task_id) : null, start_date: taskForm.start_date || null, end_date: taskForm.end_date || null }),
    });
  ...
  const updateTask = async (task: any, values: Record<string, unknown>) => {
    await apiFetch(`/api/v1/projects/${projectId}/tasks/${task.id}`, { method: 'PATCH', body: JSON.stringify(values) });
    await reloadProject();
  };

  const createMilestone = async () => {
    if (!milestoneForm.name.trim()) return;
    await apiFetch(`/api/v1/projects/${projectId}/milestones`, { method: 'POST', body: JSON.stringify({ ...milestoneForm, due_date: milestoneForm.due_date || null }) });
  ...
  const saveMilestone = async () => {
    ...
    await apiFetch(`/api/v1/projects/${projectId}/milestones/${editingMilestone.id}`, { method: 'PATCH', body: JSON.stringify({ name: editingMilestone.name, due_date: editingMilestone.due_date || null, status: editingMilestone.status }) });
  ...
  const allocateEquipment = async (taskId: number) => {
    ...
    await apiFetch(`/api/v1/equipment/${equipmentId}/allocation`, { method: 'PATCH', body: JSON.stringify({ task_id: taskId }) });
    await reloadProject();
  };
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        action={<div className="flex items-center gap-2"><StatusBadge status={projectData.status} />{role === 'admin' && projectData.status !== 'archived' && <><Button size="sm" variant="outline" onClick={() => { setEditName(projectData.name); setEditBudget(String(projectData.budget_allocated)); setEditing(true); }}>Edit</Button><Button size="sm" variant="destructive" onClick={archiveProject}>Archive</Button></>}</div>}
      />
      ...
      {(role === 'admin' || role === 'pm') && (
        <Card className="mb-6">
          <CardHeader><CardTitle className="text-lg">Manage Schedule</CardTitle></CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-3">
            <input className="h-10 rounded-sm border-2 border-border bg-card px-3 text-sm" type="date" value={projectData.start_date || ''} onChange={(event) => setProjectData({ ...projectData, start_date: event.target.value })} />
            <input className="h-10 rounded-sm border-2 border-border bg-card px-3 text-sm" type="date" value={projectData.end_date || ''} onChange={(event) => setProjectData({ ...projectData, end_date: event.target.value })} />
            <div className="flex gap-2"><select className="h-10 flex-1 rounded-sm border-2 border-border bg-card px-3 text-sm" value={projectData.status} onChange={(event) => setProjectData({ ...projectData, status: event.target.value })}><option value="planning">Planning</option><option value="in_progress">In progress</option><option value="on_hold">On hold</option><option value="completed">Completed</option></select><Button onClick={saveSchedule}>Save Schedule</Button></div>
          </CardContent>
        </Card>
      )}
